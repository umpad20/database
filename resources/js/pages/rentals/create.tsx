...
export default function RentalCreate({ motorcycle }: { motorcycle?: Motorcycle }) {
    ...
    const handleSubmit = () => {
        post('/rentals', {
            forceFormData: true,
            onError: (errors) => {
                console.error('Rental submission errors:', errors);
            },
        });
    };
                    ...
                    <div className="flex-1 space-y-8 animate-fade-in-up delay-100">
                                ...
                                {[
                                    ...
                                    return (
                                        <div key={s.num} className="flex flex-col items-center gap-3 bg-background px-2">
                                            <div className={`flex h-12 w-12 items-center justify-center rounded-full border-4 transition-all duration-300 ${isActive
                        ...
                        <div className="rounded-2xl border bg-card p-6 sm:p-8 shadow-sm relative overflow-hidden">
                            ...
                            {step === 1 && (
                                <div className="space-y-8 animate-fade-in">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className={`w-full rounded-xl border bg-background px-4 py-3.5 pl-11 text-sm outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 ${errors.start_date ? 'border-destructive' : ''}`}
                                                />
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className={`w-full rounded-xl border bg-background px-4 py-3.5 pl-11 text-sm outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 ${errors.end_date ? 'border-destructive' : ''}`}
                                                />
                                                <Calendar className="absolute left-4 top-3.5 h-5 w-5 text-muted-foreground" />
                                            </div>
                                    ...
                                    <div className="space-y-4 pt-4 border-t">
                                        <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Fulfillment Method</h3>
                                        <div className="grid grid-cols-2 gap-4">
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    setData('fulfillment_type', 'pickup');
                                                    setData('pickup_location', 'Main Shop - Butuan City');
                                                    setData('return_location', 'Main Shop - Butuan City');
                                                }}
                                                className={`flex flex-col items-center gap-2 rounded-xl border p-4 transition-all ${data.fulfillment_type === 'pickup' ? 'border-primary bg-primary/5 ring-2 ring-primary/20 shadow-sm' : 'border-muted hover:border-muted-foreground/30'}`}
                                            >
                                                <Bike className={`h-6 w-6 ${data.fulfillment_type === 'pickup' ? 'text-primary' : 'text-muted-foreground'}`} />
                                                <span className={`text-xs font-bold ${data.fulfillment_type === 'pickup' ? 'text-primary' : 'text-muted-foreground'}`}>Shop Pickup</span>
                                            </button>
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    setData('fulfillment_type', 'delivery');
                                                    setData('pickup_location', '');
                                                    setData('return_location', '');
                                                }}
                                                className={`flex flex-col items-center gap-2 rounded-xl border p-4 transition-all ${data.fulfillment_type === 'delivery' ? 'border-primary bg-primary/5 ring-2 ring-primary/20 shadow-sm' : 'border-muted hover:border-muted-foreground/30'}`}
                                            >
                                                <FileText className={`h-6 w-6 ${data.fulfillment_type === 'delivery' ? 'text-primary' : 'text-muted-foreground'}`} />
                                                <span className={`text-xs font-bold ${data.fulfillment_type === 'delivery' ? 'text-primary' : 'text-muted-foreground'}`}>Home Delivery</span>
                                            </button>
                                        </div>
                                        ...
                                        {data.fulfillment_type === 'delivery' && (
                                            <div className="space-y-4 animate-in slide-in-from-top-2 duration-200">
                                                ...
                                                <div className="space-y-2">
                                                    <label className="text-sm font-semibold">Return/Pickup Address</label>
                                                    <input
                                                        type="text"
                                                        placeholder="Enter location for bike return"
                                    ...
                                    {!selectedBike.id && (
                                        <div className="rounded-xl bg-amber-50 p-4 text-amber-800 border border-amber-100 text-sm">
                                            Please go back to the <Link href="/motorcycles" className="font-bold underline">Motorcycles</Link> page and select a unit first.
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            {step === 2 && (
                                <div className="space-y-8 animate-fade-in">
                                    <div>
                                        <h2 className="text-xl font-bold flex items-center gap-2 mb-2"><FileText className="h-5 w-5 text-primary" /> Personal & Documents</h2>
                                        <p className="text-sm text-muted-foreground">Please provide your details and upload requirements.</p>
                                    </div>
                                    ...
                                    <div className="grid gap-6 sm:grid-cols-2">
                                        <div className="space-y-2">
                                            ...
                                            <input
                                                ...
                                                className={`w-full rounded-xl border bg-background px-4 py-3 text-sm outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 ${errors.phone ? 'border-destructive' : ''}`}
                                            />
                                        ...
                                        <div className="col-span-2 space-y-2">
                                            ...
                                            <textarea
                                                ...
                                                className={`w-full rounded-xl border bg-background px-4 py-3 text-sm outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 ${errors.address ? 'border-destructive' : ''}`}
                                                rows={2}
                                            />
                                    ...
                                    <div className="grid gap-4 sm:grid-cols-2">
                                        <div className="space-y-2">
                                            <label className={`flex cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed ${data.id_document ? 'border-primary bg-primary/5' : 'border-muted'} p-8 text-center transition-all hover:border-primary hover:bg-primary/5`}>
                                                <div className="rounded-full bg-primary/10 p-3"><Upload className="h-6 w-6 text-primary" /></div>
                                                <div>
                                                    <span className="block text-sm font-semibold">{data.id_document ? data.id_document.name : 'Upload Valid ID'}</span>
                                                    <span className="block text-xs text-muted-foreground mt-1">JPG, PNG (Max 5MB)</span>
                                                </div>
                                                <input type="file" className="hidden" accept="image/*" onChange={(e) => setData('id_document', e.target.files?.[0] || null)} />
                                            </label>
                                            {errors.id_document && <p className="text-xs text-destructive text-center">{errors.id_document}</p>}
                                        </div>
                                        <div className="space-y-2">
                                            <label className={`flex cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed ${data.license_document ? 'border-primary bg-primary/5' : 'border-muted'} p-8 text-center transition-all hover:border-primary hover:bg-primary/5`}>
                                                <div className="rounded-full bg-primary/10 p-3"><Upload className="h-6 w-6 text-primary" /></div>
                                                <div>
                                                    <span className="block text-sm font-semibold">{data.license_document ? data.license_document.name : 'Driver\'s License'}</span>
                                                    <span className="block text-xs text-muted-foreground mt-1">Required for operation</span>
                                                </div>
                                                <input type="file" className="hidden" accept="image/*" onChange={(e) => setData('license_document', e.target.files?.[0] || null)} />
                                            </label>
                                            {errors.license_document && <p className="text-xs text-destructive text-center">{errors.license_document}</p>}
                                        </div>
                                    </div>
                            ...
                            {step === 3 && (
                                <div className="space-y-8 animate-fade-in">
                                        ...
                                        <div className="relative space-y-4">
                                            ...
                                            <div className="grid grid-cols-2 gap-y-4 gap-x-8 text-sm">
                                                <div>
                                                    ...
                                                    <p className="font-semibold text-base">{data.start_date}</p>
                                                </div>
                                                <div>
                                                    <p className="text-muted-foreground mb-1">Return Date</p>
                                                    <p className="font-semibold text-base">{data.end_date}</p>
                                                </div>
                                                <div>
                                                    <p className="text-muted-foreground mb-1">Duration</p>
                                                    <p className="font-semibold text-base">{days} Day(s)</p>
                                                </div>
                                                <div>
                                                    <p className="text-muted-foreground mb-1">Estimated Total</p>
                                                    <p className="font-semibold text-base">₱{totalAmount.toLocaleString()}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {Object.keys(errors).length > 0 && (
                                        <div className="rounded-xl bg-destructive/10 p-4 border border-destructive/20 animate-shake">
                                            <div className="flex items-center gap-2 text-destructive mb-2">
                                                <XCircle className="h-5 w-5" />
                                                <span className="font-bold text-sm">Please fix the following errors:</span>
                                            </div>
                                            <ul className="list-disc list-inside space-y-1">
                                                {Object.entries(errors).map(([key, error]) => (
                                                    <li key={key} className="text-xs text-destructive-foreground font-medium capitalize">
                                                        {error}
                                                    </li>
                                                ))}
                                            </ul>
                        ...
                        <div className="flex items-center justify-between pt-2">
                            <button
                                ...
                                disabled={step === 1}
                                className={`flex items-center gap-2 rounded-xl px-6 py-3 text-sm font-semibold transition-all ${step === 1 ? 'opacity-0 pointer-events-none' : 'border border-input bg-background hover:bg-muted text-foreground hover:text-foreground'
                                    }`}
                            >
                            ...
                            {step < totalSteps ? (
                                ...
                                <>
                                    {errors.motorcycle_id && <p className="text-sm text-destructive text-center mb-4">{errors.motorcycle_id}</p>}
                                    <button
                                        onClick={handleSubmit}
                                        disabled={processing}
                                        className="flex items-center gap-2 rounded-xl bg-green-600 px-8 py-3 text-sm font-semibold text-white shadow-lg shadow-green-600/25 transition-all hover:bg-green-700 hover:shadow-xl hover:-translate-y-0.5 disabled:opacity-50"
                                    >
                                        {processing ? 'Submitting...' : <><CheckCircle2 className="h-5 w-5" /> Submit Request</>}
                                    </button>
                                </>
                            )}
                        </div>
